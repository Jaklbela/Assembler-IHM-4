...
### Результаты программы при отсутствии синхропримитивов (для 9 баллов)
Замечено, что при отсутствии синхропримитивов работа потоков из-за одновременной работы приводит к единовременному выводу всех сообщений.
Следовательно, как мы можем заметить в результатах работы из файлов 1 и 3, неправильно работали переносы, сообщения одного потока выводились
поверх сообщения другого, обрезая слова, путался вывод дней. Этой ошибки не было, если присутствовал лишь один гость - следовательно, был
только один поток. Важно отметить, что даже в любом случае программа продолжает работать корректно и не пытается взять гостей, когда они
уже закончились.
